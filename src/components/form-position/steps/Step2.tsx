...
export function Step2({ outcomes, onOutcomeChange, onNext, onBack, showModal }: Step2Props) {
  const handleNext = () => {
    const allAccepted = Object.values(outcomes).every((outcome) => outcome.accepted);
    if (!allAccepted) {
      showModal('Por favor, confirme o entendimento de todos os outcomes.', 'Atenção', 'warning');
      return;
    }

    const allCommentsValid = Object.values(outcomes).every(
      (outcome) => outcome.comment.length >= 150
    );
    if (!allCommentsValid) {
      showModal('Por favor, forneça uma reflexão de pelo menos 150 caracteres para cada outcome.', 'Atenção', 'warning');
      return;
    }
  ...
  return (
    <div className="opacity-0 animate-[fadeInUp_0.5s_ease_forwards]">
      ...
      <OutcomeBlock
        ...
        description={
          <p className="mb-4 text-[#0a0e27]">
            Avaliar tecnicamente e comportamentalmente a equipe atual, promovendo treinamento
            intensivo e ajustes necessários.
          </p>
        }
        ...
        onAcceptedChange={(accepted) => onOutcomeChange('teamRestructure', { accepted })}
        onCommentChange={(comment) => onOutcomeChange('teamRestructure', { comment })}
      />
      ...
      <OutcomeBlock
        ...
        description={
          <p className="mb-4 text-[#0a0e27]">
            Garantir operação consistente, disciplinada e previsível, seguindo o playbook definido.
          </p>
        }
        ...
        onAcceptedChange={(accepted) => onOutcomeChange('operationalDiscipline', { accepted })}
        onCommentChange={(comment) => onOutcomeChange('operationalDiscipline', { comment })}
      />
      ...
      <OutcomeBlock
        ...
        description={
          <p className="mb-4 text-[#0a0e27]">
            Construir time comercial saudável, treinado e performático, capaz de sustentar
            resultados sem dependência excessiva do líder.
          </p>
        }
        accepted={outcomes.highPerformance.accepted}
        comment={outcomes.highPerformance.comment}
        onAcceptedChange={(accepted) => onOutcomeChange('highPerformance', { accepted })}
        onCommentChange={(comment) => onOutcomeChange('highPerformance', { comment })}
      />
      ...
      <OutcomeBlock
        ...
        onAcceptedChange={(accepted) => onOutcomeChange('barRaiser', { accepted })}
      ...
      <OutcomeBlock
        ...
        description={
          <p className="mb-4 text-[#0a0e27]">
            O sucesso do líder está diretamente ligado ao sucesso de seus liderados.
          </p>
        }
        ...
        onAcceptedChange={(accepted) => onOutcomeChange('accountability', { accepted })}
        onCommentChange={(comment) => onOutcomeChange('accountability', { comment })}
      />
      ...
      <OutcomeBlock
        ...
        onAcceptedChange={(accepted) => onOutcomeChange('conversion', { accepted })}
      ...
      <OutcomeBlock
        ...
        description={
          <p className="mb-4 text-[#0a0e27]">
            Implementar ao menos uma automação ou solução de IA que redesenhe processos comerciais.
          </p>
        }
